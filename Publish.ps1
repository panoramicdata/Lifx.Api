...
# Write-Host goes straight to the host, so nothing can capture, redirect or suppress it -- which is
# why PSScriptAnalyzer objects to it. Write-Information carries the same text through the
# information stream instead, and $PSStyle (PowerShell 7+) keeps the colour where there is a
# console to show it.
function Write-Status {
	...
	$text = $Message
	if ($Colour -ne 'Default' -and $PSStyle) {
